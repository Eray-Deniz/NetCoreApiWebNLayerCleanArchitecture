...
{
    //ProductService i taşıdığımızda ToListAsync() gibi ef core motodlarında hata verdi. Application' a ef core'u veritabanını ilgilendiren bir paket olduğu için yüklemedik.
    //IProductRepository ve IGenericRepository interface lerini ve aşağıdaki metodların içeriğini değiştirdik.
    ...
    public class ProductService(IProductRepository productRepository, IUnitOfWork unitOfWork, IMapper mapper) : IProductService
    {
        ...
        public async Task<ServiceResult<CreateProductResponse>> CreateAsync(CreateProductRequest request)
        {
            ...
            var isProctNameExist = await productRepository.AnyAsync(x => x.Name == request.Name);
            if (isProctNameExist)
            {
        ...
        public async Task<ServiceResult> UpdateAsync(int id, UpdateProductRequest request)
        {
            ...
            var isProctNameExist = await productRepository.AnyAsync(x => x.Name == request.Name && x.Id != id);
            if (isProctNameExist)
            {
